...
def create_credentials_element(_xmlcredentials, credentials):
    ...
    for service, credential in credentials.items():
        ...
        if cred_type == "usk":
            private = credential.get("private")
            _xmlcredential.add_element("private", private)
        else:
...
def run_monitor_scan_progress(name, scan_id,spyderisk_model=None,spyderisk_url=None,spyderisk_user=None,spyderisk_pwd=None,spyderisk_asset=None,debug=None):
    if spyderisk_model and spyderisk_asset:
        command = ["python3", "{0}/monitor_scan_progress".format(HOME_PATH), "--name", name, "--scanid", scan_id,
                   "--spyderisk_url",spyderisk_url,"--spyderisk_user",spyderisk_user,"--spyderisk_pwd",spyderisk_pwd,
                   "--spyderisk_model", spyderisk_model, "--spyderisk_asset", spyderisk_asset]
    else:
    #command = "nohup python3 {HOME_PATH}/monitor_scan_progress --name {NAME} --scanid {SCANID} 2>&1 >> /proc/1/fd/1 &".format(HOME_PATH=HOME_PATH, NAME=name, SCANID=scan_id)
        command = ["python3", "{0}/monitor_scan_progress".format(HOME_PATH), "--name", name, "--scanid", scan_id]
    if debug:
...
def create_target_list(target,sshuser=None,sshpass=None,sshkey=None,portlist=None):
    """
    Create a valid target list for the scanner
    :param target: Valid target to scan IP/DNS
    :param ssh: ssh (username,password) tuple
    :return: target_list
    """
    if portlist:
        try:
            ports = PORT_LISTS[portlist]
        except KeyError:
            raise HTTPException(status_code=404, detail="PortList not found: '{0}'".format(portlist))
    else:
        ports = PORT_LISTS["ALL_TCP_AND_NMAP_5_51_TOP_100_UDP"]
    if sshuser:
        if sshkey:
            ssh = {
                'username': sshuser,
...
def check_valid_target(target):
    """
    Take IP or DNS and check if valid
    :param target:
    :return: Bool
    """
    try:
...
def create_html_monitor(progress,scanid,name):
    """
    Generate progress HTML report to show scan progress and how to access completed scan.
    :param progress:
    :param scanid:
    :param name:
    :return:
    """
    LOGGER.debug("Create_progress start")
    template_in_progress = r'''<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>
    ...
    .progress-bar {{
        width: {PROGRESS}%; /* Change this value to indicate progress */
        height: 30px;
...
<div class="container">
    <h2>Scan Progress</h2>
    <p>Scan Name: <span id="scanName">{NAME}</span></p>
    <p>Scan ID: <span id="scanID">{SCANID}</span></p>
    <div class="progress">
    ...
    template_completed = r'''<!DOCTYPE html>
    <html lang="en">
    <head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <div class="container">
        ...
        <a href="../completed/{NAME}.html">View Completed Scan</a></br>
        <a href="../completed/{NAME}.json">View Completed Scan JSON</a>
    </div>
    </body>
    </html>'''
    if progress == 100:
    ...
    LOGGER.debug("Create_progress before write")
    with open("{0}/www/progress/progress-{1}.html".format(HOME_PATH,name), 'w', encoding='utf-8') as f:
        f.write(template.format(NAME=name,SCANID=scanid,PROGRESS=progress))

def create_html_report(xml,name=None):
    """
    Convert XML to JSON, add to the template and write to a file.
    :param xml: Scan result in xml
    :param name: Where to write the report
    :return: None
    """
    LOGGER.debug("Create_html_report start")
    ...
    <h3>Apply filter greater than...</h2>
    <div>
        <label for="severityFilter">Severity: </label>
        <input type="text" id="severityFilter" onkeyup="filterTable('severityFilter', 1)" placeholder="Filter by severity...">
        <br><br>
        <label for="qodFilter">Quality of Detection(QoD):</label>
        <input type="text" id="qodFilter" onkeyup="filterTable('qodFilter', 4)" placeholder="Filter by Quality of Detection...">
    </div>

    <h3>Vulnerabilities</h3>
    <div id="_alarmTable"></div>

    <h3>Log Messages</h3>
    <div id="_logMessageTable"></div>

    <h3>Error Messages</h3>
    <div id="_errorMessageTable"></div>
    ...
    <script>
        ...
        function jsonToTable(jsonData, type) {{
            var table = '<table id="'+type+'Table"><tr><th>Name</th><th onclick="sortTable(1)">Severity</th><th>Host</th><th>Port</th><th onclick="sortTable(4)">QoD</th><th>Text</th></tr>';
            var results = jsonData.get_scans_response.scan.results.result;
            for (var i = 0; i < results.length; i++) {{
                if (results[i]['@type'] === type) {{
                    table += '<tr>';
                    table += '<td>' + results[i]['@name'] + '</td>';
                    table += '<td>' + results[i]['@severity'] + '</td>';
                    table += '<td>' + results[i]['@host'] + '</td>';
                    table += '<td>' + results[i]['@port'] + '</td>';
                    table += '<td>' + results[i]['@qod'] + '</td>';
                    table += '<td>' + results[i]['#text'] + '</td>';
                    table += '</tr>';
                }}
            }}
            table += '</table>';
        ...
        // Function to filter table rows based on input criteria
        ...
        // Function to sort table rows based on the selected column
        ...
        document.getElementById('_alarmTable').innerHTML = jsonToTable(jsonData, "Alarm");
        sortTable(1);
        document.getElementById('_logMessageTable').innerHTML = jsonToTable(jsonData, "Log Message");
        document.getElementById('_errorMessageTable').innerHTML = jsonToTable(jsonData, "Error Message");
    </script>

    </body>
    </html>'''
    xml_dict = xmltodict.parse(etree.tostring(xml, pretty_print=True).decode('utf-8'))
    json_data = json.dumps(xml_dict)
    LOGGER.debug("Create_html_report before write")
    if name:
        with open("{0}/www/completed/{1}.html".format(HOME_PATH,name), 'w', encoding='utf-8') as f:
            f.write(template.format(JSON_DATA=json_data))
    else:
        return template.format(JSON_DATA=json_data)

#def convert_xml_to_json(xml,name=None):
#    """
#    Convert XML Etree object into JSON and write to a file
#    :param etree: etree object of the scan
#    :param filepath: Where to write the JSON file
#    :return: json object
#    """
#    LOGGER.debug("Create_json start")
#    xml_dict = xmltodict.parse(etree.tostring(xml, pretty_print=True).decode('utf-8'))
#    LOGGER.debug("Create_json before write")
#    if name:
#        with open("{0}/www/completed/{1}.json".format(HOME_PATH,name), 'w', encoding='utf-8') as f:
#            json.dump(xml_dict, f, ensure_ascii=False, indent=4)
#    else:
#        return json.dumps(xml_dict)

def convert_xml_to_json(xml, format="full", name=None):
    """
    Convert XML Etree object into JSON and write to a file
    :param xml: etree object of the scan
    :param format: Output format - "full" or "simple"
    :param name: Where to write the JSON file
    :return: json object
    """
    LOGGER.debug("Create_json start with format: {}".format(format))
    xml_dict = xmltodict.parse(etree.tostring(xml, pretty_print=True).decode('utf-8'))
    
    if format.lower() == "simple":
        LOGGER.debug("Creating simplified JSON output")
        simple_results = []
        
        # Check if results exist in the XML
        if ('get_scans_response' in xml_dict and 
            'scan' in xml_dict['get_scans_response'] and 
            'results' in xml_dict['get_scans_response']['scan'] and 
            'result' in xml_dict['get_scans_response']['scan']['results']):
            
            results = xml_dict['get_scans_response']['scan']['results']['result']
            if not isinstance(results, list):
                results = [results]  # Convert to list if it's a single item
                
            for result in results:
                # Check if it's a Log Message and severity > 0.0
                #LOGGER.debug("Result: {}".format(result.get('@severity')))
                if (result.get('@type') == 'Alarm' and 
                    float(result.get('@severity', '0.0')) > 0.0):
                    ...
                    simple_entry = {
                        ...
                        'qod': int(result.get('@qod')),
                        'text': result.get('#text', '')
                    }
                    
                    # Prepend URI if it exists and is not empty
                    uri = result.get('@uri')
                    if uri and uri.strip():
                        simple_entry['text'] = "URI: {0}".format(uri) + " - " + simple_entry['text']
                        
                    simple_results.append(simple_entry)
        
        # Sort results:
        # 1. By host name/IP ascending
        # 2. By port: TCP ports before UDP, then by port number ascending
        # 3. By severity descending (higher severity first)
        def sort_key(item):
            host = item.get('host', '')
            port = item.get('port', '')
            ...
            # TCP comes before UDP in sorting
            protocol_value = 0 if protocol == 'tcp' else 1
            
            # For severity, we negate to sort descending
            return (host, protocol_value, port_number, -severity)
        
        simple_results.sort(key=sort_key)
        output = simple_results
    else:
        # Return full JSON
        output = xml_dict
...
PORT_LISTS = {
    "SSH": "T:22",
    "NMAP_100_TCP": "T:7,9,13,21-23,25-26,37,53,79-81,88,106,110-111,113,119,135,139,143-144,179,199,389,427,443-445,465,513-515,543-544,548,554,587,631,646,873,990,993,995,1025-1029,1110,1433,1720,1723,1755,1900,2000-2001,2049,2121,2717,3000,3128,3306,3389,3986,4899,5000,5009,5051,5060,5101,5190,5357,5432,5631,5666,5800,5900,6000-6001,6646,7070,8000,8008-8009,8080-8081,8443,8888,9100,9999-10000,32768,49152-49157",
...
vt_core_selection = {
    ...
    '1.3.6.1.4.1.25623.1.0.12288': {
        "1": "0", #Strictly unauthenticated
        "2": "1", #Enable CGI scanning
        "3": "1", #Add historic /scripts and /cgi-bin to directories for CGI scanning
        "4": "/(index\.php|image|img|css|js$|js/|javascript|style|theme|icon|jquery|graphic|grafik|picture|bilder|thumbnail|media/|skins?/)", #Regex pattern to exclude directories from CGI scanning
        "5": "1", #Use regex pattern to exclude directories from CGI scanning
        "6": "1", #Exclude directories containing detected known server manuals from CGI scanning
        "7": "0", #Enable generic web application scanning
        "9": "Verbose", #Report verbosity Normal;Quiet;Verbose;Debug
        "10": "Debug", #Log verbosity Normal;Quiet;Verbose;Debug
        "11": "1", #Debug level
        "12": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36", #HTTP User-Agent
        "13": "1", #Exclude printers from scan
        "14": "0", #Exclude known fragile devices/ports from scan
        "15": "0", #Enable SSH Debug
        "17": "0", #Service discovery on non-default UDP ports (slow)
        "18": "1", #Disable caching of web pages during CGI scanning
        "19": "2" #Traversal depth to use for directory traversal attacks during CGI scanning
    },
    #host_alive_detection.nasl
    '1.3.6.1.4.1.25623.1.0.100315': {
        "1": "1", #Do a TCP ping
        "2": "1", #TCP ping tries also TCP-SYN ping
        "5": "1", #Mark unrechable Hosts as dead (not scanning)
        "6": "1", #Report about unrechable Hosts
        "8": "1", #Use nmap
        "9": "1", #Report about reachable Hosts
        "12": "1", #Log nmap output
        "13": "1", #Log failed nmap calls
        "14": "Aggressive" #nmap timing policy
        },
    #nmap.nasl
    '1.3.6.1.4.1.25623.1.0.14259': {
        "1": "SYN scan", #Service scan "connect();SYN scan;FIN scan;Xmas Tree scan;SYN FIN scan;FIN SYN scan;Null scan;No TCP scan"
        "7": "Aggressive", #Timing policy "Aggressive;Insane;Normal;Polite;Sneaky;Paranoid;Custom"
        "21": "1", #Log nmap output
        "25": "1" #No ARP or ND Ping
    },
    #find_service.nasl
    '1.3.6.1.4.1.25623.1.0.10330': {
        "1": "All", #Test SSL based services "All;None"
        "2": "8", #Number of connections done in parallel
        "3": "5", #Network connection timeout
        "4": "5", #Network read/write timeout
        "5": "10" #Wrapped service read timeout
    },
    'vt_groups': ['family=Service detection'],
    'vt_groups': ['family=Product detection'],
    '1.3.6.1.4.1.25623.1.0.810003': {}, #2010/host_summary.nasl General
    '1.3.6.1.4.1.25623.1.0.810002': {} #cpe_inventory.nasl Service detection
}


vt_groups_list = ['family=AIX Local Security Checks',
...
#                  'family=Brute force attacks',
                  ...
                  'family=F5 Local Security Checks',
                  'family=FTP',
                  ...
                  'family=Huawei EulerOS Local Security Checks',
                  'family=Huawei',
                  'family=IT-Grundschutz-15',
                  'family=IT-Grundschutz-deprecated',
                  ...
                  'family=Palo Alto PAN-OS Local Security Checks',
...
vt_selection_full_and_fast = {} #Same selection as full-and-fast-daba56c8-73ec-11df-a475-002264764cea.xml
vt_selection_full_and_fast.update(vt_core_selection)
vt_selection_full_and_fast.update({'vt_groups':vt_groups_list})


vt_selection_ssh_test = {} #Same selection as full-and-fast-daba56c8-73ec-11df-a475-002264764cea.xml
vt_selection_ssh_test.update(vt_core_selection)
#ssh_detect.nasl oid("1.3.6.1.4.1.25623.1.0.10267")
#ssh_authorization.nasl oid("1.3.6.1.4.1.25623.1.0.90022")
#gb_sudo_ssh_login_detect.nasl oid("1.3.6.1.4.1.25623.1.0.117185")
#default_ssh_credentials.nasl (1.3.6.1.4.1.25623.1.0.108013)
#ssh_login_failed.nasl oid(1.3.6.1.4.1.25623.1.0.105936)
vt_selection_ssh_test.update({'1.3.6.1.4.1.25623.1.0.10267':{},'1.3.6.1.4.1.25623.1.0.90022':{},
                              '1.3.6.1.4.1.25623.1.0.117185':{},'1.3.6.1.4.1.25623.1.0.108013':{},
                              '1.3.6.1.4.1.25623.1.0.105936':{}})
